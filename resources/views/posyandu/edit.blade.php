...
@section('content')
@php($header = 'Edit Posyandu')

<form method="post" action="{{ route('posyandu.update', $posyandu) }}" class="space-y-4">
    @csrf
    @method('PUT')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
            ...
            <select name="puskesmas_id"
                ...
                <option value="{{ $p->id }}" @selected(old('puskesmas_id', $posyandu->puskesmas_id) ==
                    $p->id)>{{ $p->name }}</option>
                @endforeach
            </select>
        ...
        <div class="md:col-span-2">
            ...
            <textarea name="address" rows="3"
                class="w-full rounded border-slate-300 focus:border-brand-400 focus:ring-brand-400">{{ old('address', $posyandu->address) }}</textarea>
            @error('address') <div class="text-xs text-rose-600 mt-1">{{ $message }}</div> @enderror
        </div>

        <div>
            <label class="block text-sm mb-1">Telepon</label>
            <input type="text" name="phone" value="{{ old('phone', $posyandu->phone) }}"
                class="w-full rounded border-slate-300 focus:border-brand-400 focus:ring-brand-400">
        ...
        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_active" value="1" id="is_active" @checked(old('is_active',
                $posyandu->is_active)) class="rounded border-slate-300">
    ...
    <div class="pt-4 flex items-center gap-2">
        ...
        <button class="px-3 py-2 rounded bg-brand-500 text-white hover:bg-brand-600">Simpan Perubahan</button>
    </div>
</form>
@endsection
